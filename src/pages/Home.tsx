...
const HOW_IT_WORKS = [
  {
    ...
    title: "도구 선택",
    desc: "7가지 무료 도구 중 필요한 것을 고르세요. 검색이나 카테고리로 빠르게 찾을 수 있어요.",
  },
  {
    step: "02",
    icon: Type,
    title: "텍스트 · 파일 입력",
    desc: "글자, 이미지, URL을 그냥 붙여넣거나 올리세요. 서버 전송 없이 브라우저에서만 처리됩니다.",
  },
  {
    step: "03",
    icon: CheckCircle2,
    title: "즉시 결과 복사 · 저장",
    desc: "결과를 바로 복사하거나 파일로 저장하세요. 로그인, 회원가입 없이 완전 무료입니다.",
  },
];
...
export default function Home() {
  ...
  const [recentTools] = useState<string[]>(() => getRecentTools());
  ...
  const favoriteToolConfigs = favorites
  ...
  const renderToolCard = (tool: ToolCardItem, showFavorite = true) => {
    ...
    return (
      <Link key={tool.id} to={tool.path} className="group relative">
        <Card
          className={`h-full transition-all hover:shadow-lg hover:scale-[1.02] hover:border-primary/50 border ${colors.border} overflow-hidden`}
        >
          {showFavorite && (
            <Button
              variant="ghost"
          ...
          <CardHeader>
            ...
            <CardDescription className="mt-1 text-xs text-muted-foreground/70 dark:text-muted-foreground">
              {tool.titleEn}
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            description:
              "국내 크리에이터를 위한 무료 온라인 도구 서비스. 로그인 없이 3초 안에 사용 가능한 QR 생성기, 글자수 세기, WebP 변환기 등.",
            ...
            mainEntity: [
              {
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "네, 크레피카의 모든 도구는 완전 무료입니다. 로그인이나 결제 없이 바로 사용할 수 있습니다.",
                },
              },
              {
                "@type": "Question",
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "아니요. 크레피카의 모든 처리는 브라우저 내에서만 이루어집니다. 입력한 데이터나 파일은 서버로 전송되지 않으며 개인정보가 완벽하게 보호됩니다.",
                },
              },
              {
                "@type": "Question",
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "네이버 블로그, 인스타그램, 유튜브, 트위터(X), 카카오 등 주요 SNS 플랫폼의 글자수 제한을 기준으로 실시간 체크가 가능합니다.",
                },
              },
              {
                "@type": "Question",
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "네이버와 같은 국내 포털은 메타 설명 등에 글자 수가 아닌 바이트 수 제한을 적용합니다. 한글 1자는 UTF-8 기준 3바이트이므로 정확한 바이트 계산이 SEO 최적화에 필수입니다.",
                },
              },
              {
                "@type": "Question",
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "JPG, PNG, GIF 등 일반 이미지 파일을 WebP 형식으로 변환할 수 있습니다. WebP는 동일 품질 대비 파일 크기가 30% 이상 작아 웹사이트 속도 개선에 효과적입니다.",
                },
              },
            ],
          })}
      ...
      <div className="container px-4 py-10 mx-auto max-w-7xl">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-3">
            네이버·인스타·유튜브 콘텐츠 작업을{" "}
            <strong className="text-foreground">3초 안에</strong> 끝내세요.
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-7 text-sm">
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted/60">
              <strong className="text-primary font-bold">
                {TOOLS_CONFIG.length}가지
              </strong>{" "}
              무료 도구
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted/60">
              <strong className="text-primary font-bold">100%</strong> 브라우저
              처리
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted/60">
              <strong className="text-primary font-bold">0원</strong> · 로그인
              불필요
            </span>
          </div>
              ...
              {top3Tools.map((tool, idx) => {
                ...
                return (
                  <Link key={tool.id} to={tool.path} className="group">
                    <Card
                      className={`h-full transition-all hover:shadow-xl hover:scale-[1.02] border-2 ${colors.border} overflow-hidden relative`}
                    >
                      <div
                        className={`absolute top-0 left-0 right-0 h-1 ${colors.bg.replace("/10", "")}`}
                        style={{
              ...
              ({ category, label, tools }) =>
                tools.length > 0 && (
                  <section key={category}>
                    <div className="flex items-center gap-3 mb-6">
                      <span
                        className={`w-3 h-3 rounded-full ${CATEGORY_COLORS[category].bg.replace("/10", "-500")}`}
                      />
                      <h2 className="text-2xl md:text-3xl font-bold">
                        {label}
                      </h2>
        ...
        <section className="mb-16 content-lazy">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
            ...
            {HOW_IT_WORKS.map(({ step, icon: Icon, title, desc }) => (
              <div
                key={step}
                className="text-center p-6 rounded-2xl border bg-card/50 relative"
              >
                <div className="w-12 h-12 rounded-full bg-cyan-700 text-white flex items-center justify-center text-lg font-bold mx-auto mb-4 shadow-md">
                  {step}
                </div>
        ...
        <section className="mb-12 text-center content-lazy">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto mb-10">
            ...
            제공합니다. 소셜미디어 콘텐츠 최적화, 웹 성능을 위한 이미지 변환,
            마케팅용 QR 코드 생성 등 빠르고 스마트한 작업 환경을 만들어
            드립니다.
          </p>
            ...
            {[
              {
                ...
                title: "즉시 결과 확인",
                desc: "모든 처리가 브라우저에서 이루어집니다. 기다림 없이, 업로드 없이, 몇 초 만에 결과를 확인하세요.",
              },
              {
                icon: Lock,
                color: "bg-blue-500/10 text-blue-500",
                title: "100% 프라이빗",
                desc: "데이터가 기기 밖으로 나가지 않습니다. 서버 없음, 저장 없음. 완벽한 개인정보 보호가 보장됩니다.",
              },
              {
                icon: Gift,
                color: "bg-green-500/10 text-green-500",
                title: "항상 무료",
                desc: "구독료 없음, 숨겨진 비용 없음, 프리미엄 없음. 필요할 때마다 원하는 만큼 사용하세요.",
              },
        ...
        <section className="mb-16 content-lazy">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-4xl mx-auto">
            {[
              {
                title: "소셜미디어 마케터",
                desc: "인스타그램 캡션 포맷팅, 해시태그 섞기, 최적 게시물 길이를 위한 글자 수 세기, 플랫폼 요구사항에 맞는 콘텐츠 작성을 쉽게 하세요.",
              },
              {
                title: "웹 개발자",
                desc: "빠른 페이지 로딩을 위한 WebP 이미지 변환, 모바일 리다이렉트용 QR 코드 생성, 더 나은 SEO 성능을 위한 콘텐츠 최적화를 손쉽게 처리하세요.",
              },
              {
                title: "콘텐츠 크리에이터",
                desc: "목업용 로렘 입숨 생성, 네이버 최적화를 위한 한글 바이트 계산, 다양한 플랫폼 사양에 맞는 콘텐츠 준비를 빠르게 완료하세요.",
              },
              {
                title: "마케팅 팀",
                desc: "캠페인용 QR 코드 생성, 이메일 마케팅용 이미지 최적화, 퍼블리싱 전 모든 콘텐츠를 완벽하게 포맷팅하세요.",
              },
            ].map(({ title, desc }) => (
              <div
                key={title}
